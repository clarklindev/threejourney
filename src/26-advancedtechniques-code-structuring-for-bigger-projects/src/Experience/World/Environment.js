...
export default class Environment{
  ...
  setEnvironmentMap(){
    ...
    // this.environmentMap.texture.encoding = THREE.sRGBEncoding;  //DEPRECATED: .encoding AND THREE.sRGBEncoding
    this.environmentMap.texture.colorSpace = THREE.SRGBColorSpace; //UPDATE
    ...
    //debug
    if(this.debug.active){
      this.debugFolder.add(this.environmentMap, 'intensity').name('envMapIntensity').min(0).max(4).step(0.001).onChange(this.environmentMap.updateMaterials);//UPDATE ALL MATERIALS
    }
